...
import 'package:aura_app/services/user_service.dart';
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void initState() {
    ...
    _postCountFuture = Future.value(0);
  }
  ...
  Future<AppUser?> _loadUser() async {
    try {
      ...
      if (user.id != null) {
        _postCountFuture = PostService().getUserPosts(user.id!, page: 0, size: 1)
            .then((posts) => posts.length);
      }
      return user;
    } catch (_) {
      final mock = await MockApiService().getUserProfile();
