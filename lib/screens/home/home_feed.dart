...
import 'package:aura_app/services/mock_api_service.dart';
import 'package:aura_app/services/story_service.dart';
...
import 'package:aura_app/widgets/video_player_widget.dart';
import 'package:aura_app/widgets/shatter_overlay.dart';
import 'package:aura_app/screens/post/create_post_screen.dart';
...
class _HomeFeedState extends State<HomeFeed> {
  ...
  void initState() {
    super.initState();
    _loadPosts();
    _storiesFuture = StoryService().getActiveStories().then((stories) async {
      if (stories.isEmpty) return await MockApiService().getMockStories();
      return stories;
    }).catchError((_) => MockApiService().getMockStories());
  }

  void _loadPosts() {
    setState(() {
      _postsFuture = PostService().fetchPosts().then((supabasePosts) async {
        if (supabasePosts.isEmpty) {
          // If Supabase is empty, fall back to Mock images
          return await MockApiService().getMockPosts();
        }
        return supabasePosts;
      }).catchError((e) async {
        // If there's an error (like table not found), also fall back to Mock
        return await MockApiService().getMockPosts();
      });
    });
  }
            ...
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _buildStoryItem(story.user?.avatarUrl, story.user?.username ?? 'User');
                },
              );
            },
          ),
    ...
    return GlassContainer(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            height: 400, // Slightly taller for more "aesthetic" look
